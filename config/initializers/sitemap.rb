DynamicSitemaps::Sitemap.draw do
  ...
  Product.all.each do |product|
    url product_url(product), :last_mod => product.updated_at, :change_freq => 'daily', :priority => 1
  end
  
  #new_page!
  
  ProductSample.all.each do |sample|
    url sample_url(sample), :last_mod => sample.updated_at, :change_freq => 'daily', :priority => 1
  end
